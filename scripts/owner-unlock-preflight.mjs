...
const writeLocalEnvTemplate = async (targetPack) => {
  ...
  const reminderMarker = '# AGL_TEMPLATE_BLANK_VALUES_REMINDER'
  const reminderLines = blankKeys.length && !existingRaw.includes(reminderMarker)
    ? [
        reminderMarker,
        '# TODO: Fill the blank values below (they are currently empty).',
        '# Secrets should be stored in GitHub repository secrets/variables, not committed.',
