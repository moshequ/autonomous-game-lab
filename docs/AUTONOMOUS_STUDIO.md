...
27. iOS handoff prepares Capacitor metadata, App Store privacy/review evidence, and payback gates without creating an Apple account or Xcode project.
28. Winning games graduate to Android, then iOS when revenue justifies it.
...
- Autonomous cadence report that mirrors and locally confirms the daily Codex app automation when available, scheduled CI runner, operate command, and zero-spend recovery policy
...
- iOS App Store handoff plan with Capacitor metadata, App Privacy labels, screenshots, native-value evidence, and Apple/payback blockers
...
- `npm run autonomous:android-signing` prepares local ignored Android TWA signing material, redacted CI-secret readiness, and the public SHA-256 certificate fingerprint.
- `npm run autonomous:android-release-plan` writes the Android TWA release gate and internal-testing blockers.
- `npm run autonomous:ios-release-plan` writes the iOS App Store handoff, Capacitor draft metadata, App Review checklist, and Apple/payback blockers without generating an Xcode project.
