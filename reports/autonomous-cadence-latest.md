...
Generated: 2026-05-20T05:47:03.229Z
...
- fresh: production-environment - data/production-environment.json is 1.66h old.
- fresh: event-collector-deployment - data/event-collector-deployment.json is 0.05h old.
- fresh: event-collector-smoke - data/event-collector-smoke.json is 0.05h old.
- fresh: local-event-bridge - data/local-event-bridge.json is 0.05h old.
- fresh: event-ingest - data/event-ingest.json is 0.43h old.
- fresh: event-ingest-smoke - data/event-ingest-smoke.json is 0.05h old.
- fresh: analytics-rollup - data/analytics-rollup.json is 0.43h old.
- fresh: experiment-results - data/experiment-results.json is 1.56h old.
- fresh: growth-plan - data/growth-plan.json is 0.27h old.
- fresh: portfolio-policy - data/portfolio-policy.json is 0.27h old.
- fresh: traffic-seeding - data/traffic-seeding.json is 0.27h old.
- fresh: acquisition-learning - data/acquisition-learning.json is 0.27h old.
- fresh: organic-seed-loop - data/organic-seed-loop.json is 0.27h old.
- fresh: retention-loop - data/retention-loop.json is 0.43h old.
...
- fresh: release-health - data/release-health.json is 1.56h old.
- fresh: product-optimization - data/product-optimization.json is 0.45h old.
...
- fresh: completion-loop - data/completion-loop.json is 1.56h old.
- fresh: replay-loop - data/replay-loop.json is 1.56h old.
- fresh: first-move-coach - data/first-move-coach.json is 1.56h old.
- fresh: pwa-install-loop - data/pwa-install-loop.json is 0.43h old.
- fresh: applied-improvements - data/applied-improvements.json is 1.56h old.
- fresh: improvement-backlog - data/improvement-backlog-summary.json is 0.45h old.
- fresh: improvement-routing - data/improvement-routing.json is 0.45h old.
...
- pass: automation-verifier - test:automation is node scripts/event-collector-smoke.mjs && npm run autonomous:collector-deploy-plan && node scripts/event-ingest-smoke.mjs && node scripts/local-event-bridge.mjs && npm run autonomous:gate-recovery && npm run autonomous:sample-plan && node scripts/verify-autonomy.mjs.
- pass: browser-smoke - test:e2e is playwright test.
- pass: fresh-generated-evidence - All 36 required generated evidence artifacts are fresh within 36h.
