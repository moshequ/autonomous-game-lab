...
test('portal loads a playable canvas and autonomy cockpit', async ({ page }) => {
  ...
  const ownerLoopPanel = page.locator('.sectionPanel').filter({
    has: page.getByRole('heading', { name: 'Owner Loop' }),
  })
  ...
  await expect(ownerLoopPanel).toContainText('owner-loop-ready')
  await expect(ownerLoopPanel).toContainText(ownerLoop.mode)
  await expect(ownerLoopPanel).toContainText(ownerLoop.ownerDecision.nextBestActionId)
  await expect(ownerLoopPanel).toContainText('Backoff')
  await expect(ownerLoopPanel).toContainText(ownerLoop.executionBackoff?.status ?? 'idle')
  await expect(ownerLoopPanel).toContainText('Held local actions')
  await expect(ownerLoopPanel).toContainText(String(ownerLoop.executionBackoff?.heldActionCount ?? 0))
  await expect(ownerLoopPanel).toContainText('Ready after repeat guard')
  await expect(ownerLoopPanel).toContainText(String(ownerLoop.executionBackoff?.executableWithoutRepeatCount ?? 0))
  if (ownerLoop.externalInputHandoff) {
    await expect(ownerLoopPanel).toContainText(ownerLoop.externalInputHandoff.nextUnlockId ?? 'none')
    await expect(ownerLoopPanel).toContainText(ownerLoop.externalInputHandoff.recommendedPathId ?? 'none')
    await expect(ownerLoopPanel).toContainText(ownerLoop.externalInputHandoff.lowestInputPathId ?? 'none')
    await expect(ownerLoopPanel).toContainText(String(ownerLoop.externalInputHandoff.ownerActionRequired))
  }
  if (ownerLoop.storeExternalInputHandoff) {
    await expect(ownerLoopPanel).toContainText(ownerLoop.storeExternalInputHandoff.nextUnlockId ?? 'none')
    await expect(ownerLoopPanel).toContainText(ownerLoop.storeExternalInputHandoff.lowestInputUnlockId ?? 'none')
    await expect(ownerLoopPanel).toContainText(String(ownerLoop.storeExternalInputHandoff.ownerActionRequired))
  }
  ...
  await expect(page.getByLabel('Production Measurement')).toContainText('Template helper')
  ...
  await expect(page.getByLabel('Production Measurement')).toContainText(
    productionMeasurement.ownerUnlockPreflight?.combinedOwnerInputPreflight?.commands
      ?.setupWriteLocalEnvTemplate ?? 'none',
  )
  await expect(page.getByLabel('Production Measurement')).toContainText(
...
test('production bootstrap emits zero-spend setup handoff artifacts', async ({ page }) => {
  const bootstrap = JSON.parse(await readFile('data/production-bootstrap.json', 'utf8')) as {
    ...
    setupScript: {
      ...
      printsOwnerUnlockBrief: boolean
  ...
  expect(bootstrap.setupScript.printsOwnerUnlockBrief).toBe(true)
...
test('autonomous cadence keeps unattended operation auditable and guarded', async ({ page }) => {
  ...
  expect(cadence.commandPlan.verifyAutomation).toBe('npm run autonomous:verify')
  expect(packageJson.scripts['autonomous:verify']).toBe('npm run test:automation')
...
test('production measurement status publishes public aggregate evidence handoff', async ({ page }) => {
  ...
  const ownerUnlockPreflight = JSON.parse(await readFile('data/owner-unlock-preflight.json', 'utf8')) as {
    ...
    controls: {
      ...
      localTemplateWritePreservesExistingValues: boolean
      localTemplateWriteNoGithubMutation: boolean
      localTemplateWriteGitignored: boolean
    }
  }
  const publicOwnerUnlockPreflight = JSON.parse(
    await readFile('public/owner-unlock-preflight.json', 'utf8'),
  ) as typeof ownerUnlockPreflight
  ...
  expect(analyticsUnlockPage.publicRoutes.ownerUnlockPreflightJson).toBe('/owner-unlock-preflight.json')
  ...
  expect(ownerUnlockBrief.controls.zeroPaidSpend).toBe(true)
  ...
  expect(publicOwnerUnlockPreflight).toEqual(ownerUnlockPreflight)
  ...
  expect(combinedOwnerInputPack?.controls.localTemplateWriteNoSecretValues).toBe(true)
  expect(combinedOwnerInputPack?.controls.localTemplateWritePreservesExistingValues).toBe(true)
  expect(combinedOwnerInputPack?.controls.localTemplateWriteNoGithubMutation).toBe(true)
  ...
  expect(ownerUnlockPreflight.ownerInputPack?.controls.localTemplateWritePreservesExistingValues).toBe(true)
  ...
  expect(ownerInputActionPack.valueValidation).toMatchObject({
    id: 'browser-local-zero-secret-owner-input-check',
    status: 'ready',
    filledDownloadFileName: 'agl-owner-input-filled.env',
  })
  expect(ownerInputActionPack.valueValidation.fields).toEqual([
    expect.objectContaining({
  ...
  expect(ownerUnlockPreflight.controls.localTemplateWriteNoSecretValues).toBe(true)
  expect(ownerUnlockPreflight.controls.localTemplateWritePreservesExistingValues).toBe(true)
  expect(ownerUnlockPreflight.controls.localTemplateWriteNoGithubMutation).toBe(true)
  expect(ownerUnlockPreflight.controls.localTemplateWriteGitignored).toBe(true)
  ...
  expect(filledTemplateDownload.suggestedFilename()).toBe('agl-owner-input-filled.env')
  const filledTemplatePath = await filledTemplateDownload.path()
  if (!filledTemplatePath) {
    throw new Error('Expected filled owner input template download path.')
  }
  const filledTemplate = await readFile(filledTemplatePath, 'utf8')
  expect(filledTemplate).toContain('VITE_POSTHOG_KEY=phc_public_smoke_key\n')
  expect(filledTemplate).toContain('AGL_SUPPORT_EMAIL=support@example.com\n')
  expect(filledTemplate).not.toContain('VITE_POSTHOG_HOST=')
  const filledOwnerInputReceipt = await page.evaluate(() =>
    JSON.parse(window.localStorage.getItem('agl.ownerInputActionReceipt') ?? '{}'),
  )
  expect(filledOwnerInputReceipt).toMatchObject({
  ...
  expect(filledOwnerInputReceipt.validatedInputNames).toEqual([
    'VITE_POSTHOG_KEY',
    'AGL_SUPPORT_EMAIL',
  ])
